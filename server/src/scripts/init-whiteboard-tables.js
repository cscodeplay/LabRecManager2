...
async function initTables() {
    try {
        ...
        try {
            ...
            // Ensure Multi-Cloud, EZWrite & Whiteboard Controls plan exists
            const mcPlanCheck = await prisma.$queryRawUnsafe(`
                SELECT id FROM "implementation_plans" WHERE title ILIKE '%Multi-Cloud Storage%' LIMIT 1
            `);
            if (!mcPlanCheck || mcPlanCheck.length === 0) {
                console.log('Inserting Multi-Cloud & EZWrite Whiteboard implementation plan...');
                const mcTasks = JSON.stringify([
                ...
                const mcMetadata = JSON.stringify({
                ...
                await prisma.$executeRawUnsafe(`
                    INSERT INTO "implementation_plans" (
                        ...
                        $1, $2, $3, 'completed',
                        CURRENT_TIMESTAMP - INTERVAL '1 hour', CURRENT_TIMESTAMP, $4::jsonb, $5, $6::jsonb, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                    )
                `,
                    'Multi-Cloud Storage (Google/OneDrive/iCloud), EZWrite & myViewBoard Tools & Whiteboard Power Controls',
