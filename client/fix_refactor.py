...
def refactor_whiteboard():
    ...
    # We look for the closing of showColorPicker block
    closing_str = '                            </div>\n                        )}'
    colors_popup_end = code.find(closing_str, colors_popup_start)
    if colors_popup_end == -1:
        print("Could not find closing of showColorPicker")
        return
    colors_popup_end += len(closing_str)
