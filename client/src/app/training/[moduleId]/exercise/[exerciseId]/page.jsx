...
export default function ExerciseEditorPage() {
    ...
    const isUnitUnlocked = useCallback((unitIdx) => {
        ...
        if (!moduleData?.units) return true;
        const prevUnit = moduleData.units[unitIdx - 1];
        if (!prevUnit) return true;

        const threshold = prevUnit.unlockThreshold || 70;
        const mastery = unitMasteries.find(m => m.unitId === prevUnit.id);
        if (mastery) {
            if (mastery.status === 'mastered') return true;
            if (typeof mastery.masteryScore === 'number' && mastery.masteryScore >= threshold) {
                return true;
            }
        }

        // Fallback check against previous unit's exercises passed ratio
        if (prevUnit.exercises && prevUnit.exercises.length > 0) {
            const passedCount = prevUnit.exercises.filter(e => e.userStatus === 'passed').length;
            const pct = (passedCount / prevUnit.exercises.length) * 100;
            return pct >= threshold;
        }

        return false;
    }, [moduleData, unitMasteries]);
    ...
    // Active unit index and locked status
    const activeUnitIndex = useMemo(() => {
        if (!moduleData?.units || !exercise) return 0;
        const idx = moduleData.units.findIndex(u => u.exercises?.some(e => e.id === exercise.id) || u.id === exercise.unitId);
        return idx >= 0 ? idx : 0;
    }, [moduleData, exercise]);

    const isActiveUnitLocked = useMemo(() => {
        if (activeUnitIndex <= 0) return false;
        return !isUnitUnlocked(activeUnitIndex);
    }, [activeUnitIndex, isUnitUnlocked]);
    ...
    return (
        <div className="dark h-screen flex flex-col bg-slate-900 border-t-4 border-indigo-500 overflow-hidden text-slate-100">
            ...
            {isActiveUnitLocked && (
                <div className="bg-amber-950/90 border-b border-amber-500/40 text-amber-200 px-5 py-2.5 text-xs flex items-center justify-between z-20 shrink-0 shadow-md">
                    <div className="flex items-center gap-2.5 min-w-0">
                        <Lock className="w-4 h-4 text-amber-400 shrink-0" />
                        <span className="truncate">
                            <strong>Unit {activeUnitIndex + 1} is Locked:</strong> This unit requires ≥{moduleData?.units?.[activeUnitIndex - 1]?.unlockThreshold || 70}% mastery on Unit {activeUnitIndex}.
                        </span>
                    </div>
                    <button
                        type="button"
                        onClick={() => router.push(`/training/${moduleId}`)}
                        className="btn bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs py-1 px-3 rounded-lg shrink-0 ml-3"
                    >
                    ...
                    <div className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col z-30 shrink-0 shadow-2xl animate-in slide-in-from-left duration-200">
                        ...
                        <div className="flex-1 overflow-y-auto p-3 space-y-4">
                            {moduleData?.units?.map((unit, uIdx) => {
                                ...
                                const prevUnit = uIdx > 0 ? moduleData?.units?.[uIdx - 1] : null;

                                return (
                                <div key={unit.id} className="space-y-1.5">
                                    <div className={`flex items-center justify-between text-xs px-2 py-1 font-bold ${unlocked ? 'text-slate-400' : 'text-slate-500'}`}>
                                        <span className="truncate flex items-center gap-1.5">
                                            ...
                                            <span>Unit {unit.unitNumber}: {unit.title?.replace(/^(?:unit\s+\d+[:\s-]*)+/i, '')}</span>
                                        </span>
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                                            ...
                                            {unlocked ? `≥${unit.unlockThreshold}%` : `Locked (≥${prevUnit?.unlockThreshold || 70}%)`}
                                        </span>
                                    </div>

                                    {/* Stage 1: Pre-Lab Theory & Lesson Link */}
                                    {unlocked && (
                                        <button
                                            type="button"
                                            onClick={() => {
                                                router.push(`/training/${moduleId}/unit/${unit.id}/theory`);
                                                setIsNavOpen(false);
                                            }}
                                            className="w-full text-left p-2 rounded-xl text-xs font-semibold transition flex items-center justify-between gap-2 bg-indigo-950/40 hover:bg-indigo-900/50 border border-indigo-500/30 text-indigo-300"
                                            title="Read Pre-Lab Theory & Complete Interactive Mini-Checks"
                                        >
                                            <div className="flex items-center gap-2 min-w-0">
                                                <BookOpen className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                                                <span className="truncate">Stage 1: Pre-Lab Theory & Notes</span>
                                            </div>
                                            <ChevronRight className="w-3.5 h-3.5 text-indigo-400/70 shrink-0" />
                                        </button>
                                    )}

                                    {/* Stage 2 & 3: Practice Labs & Unit Tests */}
                                    <div className="space-y-1">
                                        {unit.exercises?.map((exItem, exIdx) => {
                                            ...
                                            if (!unlocked) {
                                                cardBg = 'bg-slate-900/30 border border-slate-800/60 text-slate-500 cursor-not-allowed opacity-50';
                                            } else if (isCurrent) {
                                                cardBg = isPassed 
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        if (!unlocked) {
                                                            toast.error(`🔒 Unit ${unit.unitNumber} is locked. Score ≥${prevUnit?.unlockThreshold || 70}% on Unit ${unit.unitNumber - 1} to unlock.`);
                                                            return;
                                                        }
                                                    ...
                                                    title={!unlocked ? `Unit ${unit.unitNumber} locked: Requires ≥${prevUnit?.unlockThreshold || 70}% on Unit ${unit.unitNumber - 1}` : exItem.title}
                                                >
                                                    <div className="flex items-center gap-2 min-w-0">
                                                        ...
                                                        <span className="truncate">
                                                            {exItem.isReviewExercise ? `📝 ${exItem.title}` : exItem.title}
                                                        </span>
                                                    </div>
