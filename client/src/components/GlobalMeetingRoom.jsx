...
export default function GlobalMeetingRoom() {
    ...
    const { activeMeeting, setActiveMeeting, isPiPVisible } = useGlobalMeeting();
    const pathname = usePathname();
    const router = useRouter();
    const params = useParams();
    const { user, isAuthenticated, _hasHydrated } = useAuthStore();
        
    // Fallback to useParams if activeMeeting is not set yet but we are on the route
    const code = activeMeeting?.roomCode || params?.code;
    
    const isFullScreen = pathname === `/meeting/${code}`;
    const showPiP = activeMeeting && !isFullScreen;
    ...
    const handlePiPMouseDown = (e) => {
        ...
        dragRef.current = { startX: e.clientX, startY: e.clientY, initialX: pipPosition.x, initialY: pipPosition.y };
    };
            ...
            <>
                ...
                <div className="relative w-full h-full bg-slate-950 flex items-center justify-center overflow-hidden">
                    {activeSpace === 'whiteboard' ? (
                        ...
                        <VideoTile
                            stream={pipStream}
                            isLocal={pipStream === localStream}
                            name={pipTitle}
                            isMuted={true} 
                        />
                    ) : (
                         <div className="flex flex-col items-center justify-center p-4 text-center">
                            <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-emerald-600 to-emerald-400 flex items-center justify-center text-white font-bold text-lg shadow-lg mb-1">
                                {session?.title ? session.title[0].toUpperCase() : 'M'}
                            </div>
